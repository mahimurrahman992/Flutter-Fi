...
class ProjectProvider with ChangeNotifier {
  ...
  Stream<QuerySnapshot> getProjects(String userId) {
    return FirebaseFirestore.instance
        .collection('projects')
        .where('user_id', isEqualTo: userId) // Filter projects by user ID
        .snapshots(); // This will return a stream of the projects
  }
...
class ProjectProvider with ChangeNotifier {
  ...
  Future<void> uploadProject(
    ...
    try {
      // First add to user's personal projects collection
      await FirebaseFirestore.instance
          .collection('users')
          .doc(userId)
          .collection('projects')
      ...
      // Then add to global projects collection
      await FirebaseFirestore.instance.collection('projects').add({
        'title': title,
        'description': description,
        'status': status,
